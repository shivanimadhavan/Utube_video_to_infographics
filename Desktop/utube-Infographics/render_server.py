...
def render_infographic(spec: dict) -> str:
    ...
    text_col  = palette.get("text",       "#ffffff")
    ...
    draw.text((padding, 25), spec.get("title", "Infographic"),
              font=fonts["title"], fill=hex_to_rgb(primary))
    y = 110
        ...
        if block_type in ("bullets", "list"):
            for item in content:
                ...
                for line in wrap_text(f"  • {item}", max_chars):
                    draw.text((padding, y), line, font=fonts["body"], fill=hex_to_rgb(text_col))
                    y += 26
            ...
            x_pos = padding
            for item in content:
                draw.text((x_pos, y), str(item), font=fonts["heading"], fill=hex_to_rgb(primary))
                x_pos += 220
    ...
    draw.text((padding, height - 30), "Generated by utube-to-infographics",
              font=fonts["small"], fill=hex_to_rgb(text_col))
